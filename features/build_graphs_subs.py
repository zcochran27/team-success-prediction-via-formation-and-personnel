...
#   [4:7] edge type one-hot   formation / sub / matchup
...
#   [10:14] stat-diff block   only present when use_stats=True (4 cols)
EDGE_BASE_DIM = 10
...
def _resolve_subs_to_starters(
    row: pd.Series, side: Side, half_duration: float,
) -> list[int]:
    ...
    ``starter_duration`` is closest to the sub's ``sub_start_min`` (within
    :data:`EDGE_TIMING_TOLERANCE_MIN`), preferring same-position matches
    when the timing tie is close. Subs are processed in chronological
    order so earlier subs reserve their starter first.
    ...
    *or* that can't be matched to a starter (rare; usually a data quirk).
    ...
    for k in sub_order:
        ...
        # their end-min (= duration since they all started at 0) is within
        # tolerance of the sub's start_min.
...
def _build_team_subgraph(
    ...
    matched_starter = _resolve_subs_to_starters(row, side, half_duration)
    ...
    for node_offset, sub_k in enumerate(sub_slots):
        ...
        smin = float(row[f"{side}_sub_start_min_{sub_k+1}"])
        dur = float(row[f"{side}_sub_duration_{sub_k+1}"])
        end_min = smin + dur
        x_num[node, 0] = 0.0
        x_num[node, 1] = dur / half_duration
        x_num[node, 2] = smin / half_duration
        x_num[node, 3] = end_min / half_duration
        x_num[node, 4] = 1.0 if end_min < half_duration - 0.05 else 0.0
    ...
    # ---- Edges ------------------------------------------------------------
    # 1) Formation edges among template slots (starter graph). Sub-inherited
    #    formation edges are emitted simultaneously: for each (i, j) in the
    #    formation, we connect every node in template_slot i to every node
    #    in template_slot j.
    src_list: list[int] = []
    dst_list: list[int] = []
    type_list: list[int] = []
    ...
    for ti, tj in formation_edges(formation):
        for a in nodes_in_template_slot[ti]:
            for b in nodes_in_template_slot[tj]:
                src_list.append(a); dst_list.append(b)
                src_list.append(b); dst_list.append(a)
                type_list.extend([EDGE_TYPE_FORMATION, EDGE_TYPE_FORMATION])
                sub_min_list.extend([0.0, 0.0])
                same_pos_list.extend([0.0, 0.0])
                overlap_list.extend([0.0, 0.0])

    # 2) Sub edges (bidirectional) between sub and the starter they replaced.
    for sub_k, st in zip(sub_slots, [matched_starter[k] for k in sub_slots]):
        if st < 0:
            continue
        sub_node = sub_node_idx[sub_k]
        # template_slot of the replaced starter.
        template_slot = aligned_starter_idx.index(st)
        smin = float(row[f"{side}_sub_start_min_{sub_k+1}"]) / half_duration
        # Same-position iff sub's recorded position matches the template's slot
        # label at the replaced starter's template slot.
...
def _build_matchup_edges(
    ...
    """Return matchup ``edge_index``, edge_type, sub_min, same_pos, overlap, same_team.

    Every (i, j) pair with overlap > 0 emits a bidirectional edge.
    """
    ...
    if n1 == 0 or n2 == 0:
        ...
        return (
            ...
            empty_l, empty_f, empty_f, empty_f, empty_f,
        )
    ...
    return edge_index, edge_type, sub_min, same_pos, overlap_norm, same_team
...
def build_half_subs_graph(
    ...
    *,
    ...
    team1_block = _build_team_subgraph(
        row, "team1", kind, use_stats, use_coords, half_duration,
    )
    if mode == "single":
        team2_block = _build_team_subgraph(
            row, "team2", kind, use_stats, use_coords, half_duration,
        )
        return (
    ...
    pos = torch.cat([team1_block["x_pos"], team2_block["x_pos"]], dim=0)
    if use_stats:
    ...
    matchup_ei, matchup_type, matchup_sub_min, matchup_same_pos, matchup_overlap, matchup_same_team = (
    ...
    edge_attr = _finalize_edge_attr(
        ...
        stats=x_stats_combined if use_stats else None,
    )
