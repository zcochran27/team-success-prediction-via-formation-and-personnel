...
partially predicted from formation and personnel choices alone. This can 
be applied to both game time roster selection and recruiting choices.
  \end{abstract}
  \vspace{1.5em}
  \end{@twocolumnfalse}
]
...
\begin{enumerate}
  ...
  \item \textbf{Opponent conditioning.} Does adding the opponent's
        lineup and personnel improve prediction (matchup vs.\ single)?
...
\subsection{Data}
The data comes from Wyscout and covers more than
...
The baseline is a gradient-boosted tree model trained on
...
\begin{figure}[H]
\centering
\refstepcounter{figure}\label{fig:graph-modes}
...
\begin{itemize}
  ...
  \item \textbf{Opponent conditioning.} \texttt{single} (single-team graph)
