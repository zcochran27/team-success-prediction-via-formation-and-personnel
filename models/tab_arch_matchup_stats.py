"""Model 4+stats -- Tabular, archetypes, matchup, with per-season stats.

Same column contract as :mod:`models.tab_arch_matchup` (both teams'
formations + 22 archetype labels + ``team1_is_home``) plus 22 × 10
per-season stat vectors flattened across slot × stat. The stat block
adds 220 numeric columns.

The fully-enriched tabular variant. Comparing this against the matchup
GNN variants (Models 7+stats / 8+stats) isolates the contribution of
the graph topology *after* every per-player signal is already
available to the model -- i.e. whether explicit positional adjacency
buys anything once XGBoost can see all the same numeric features.
"""
